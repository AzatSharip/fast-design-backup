...
@bot.message_handler(commands = ['add_designer'])
def add_designer(message):
    p_chat_id = message.chat.id
    p_user_id = re.sub('\D+', '', message.text)
    if p_user_id:
        keyboard = types.InlineKeyboardMarkup()
        callback_button = types.InlineKeyboardButton(text="Удалить дизайнера", callback_data="del_admin " + str(p_user_id))
        keyboard.add(callback_button)
        try:
            Admin.create(user_id = p_user_id)
            if p_user_id:
                bot.send_message(p_chat_id,"Пользователь с ID=" + str(p_user_id) + ", успешно добавлен в дизайнеры", reply_markup=keyboard)
                keyboard_designet = types.ReplyKeyboardRemove()
                bot.send_message(p_user_id, "Поздравляем, Вас добавили в список дизайнеров", keyboard = keyboard_designet)
        except:
            user = Admin.select().where(Admin.user_id == p_user_id).get()
            user.superuser = 0
            if user.save():
                bot.send_message(p_chat_id, "Пользователь с ID=" + str(p_user_id) + ", изменен на дизайнера", reply_markup=keyboard)
...
def get_users_list(message):
    ...
    if p_user_id:
        for user in Admin.select():
            user_type = ''
            if str(user.superuser) == '1':
                user_type = '(Админ)'
            buttons = types.InlineKeyboardButton(text="Удалить пользователя ID = " + str(user.user_id) + " " + user_type,
...
def del_admin(call):
    ...
    add_designer = re.match(r'add_designer', text)
    if del_admin:
        if p_user_id: 
            try:
                ...
                if del_user.delete_instance():
                    ...
                    bot.send_message(p_user_id, "Вы были удалены из списока дизайнеров, для общения с нажмите /start")
    ...
    if add_designer:
        if p_user_id:
            keyboard = types.InlineKeyboardMarkup()
            callback_button = types.InlineKeyboardButton(text="Удалить дизайнера", 
            callback_data="del_admin " + str(p_user_id))
        keyboard.add(callback_button)
        try:
            ...
            if user_id:
                bot.send_message(p_chat_id,
                    "Пользователь с ID=" + str(user_id) + ", успешно добавлен в дизайнеры", 
                    reply_markup=keyboard)
                keyboard_designet = types.ReplyKeyboardRemove()
                bot.send_message(user_id, "Поздравляем, вас добавили в список дизайнеров", reply_markup = keyboard_designet)
        except:
            user = Admin.select().where(Admin.user_id == p_user_id).get()
            user.superuser = 0
            if user.save():
                bot.send_message(p_chat_id,
                                 "Суперпользователь с ID=" + str(p_user_id) + ", изменен на дизайнера",
                                 reply_markup=keyboard)
...
def start(message):
    ...
    i_designer_button = types.KeyboardButton(bs.i_designer)
    markup.add(checkout_button)
    markup.add(i_designer_button)
...
def reply(message):
    ...
    users = Admin.select().where(Admin.superuser == 1).get()
    user_admin = users.user_id
    if user_admin:
        if message.text == bs.designers:
            get_users_list(message)
        else:
            k_users = types.ReplyKeyboardMarkup(resize_keyboard = True, row_width = 2)
            add_job = types.KeyboardButton(bs.add_job)
            add_msg = types.KeyboardButton(bs.add_msg)
            b_users = types.KeyboardButton(bs.designers)
            k_users.add(add_job, add_msg, b_users)
            bot.send_message(sender_id, 'Вы являетесь администратором бота, Пожалуйста, выберите действие:', reply_markup = k_users)
    else:
        ...
        if message.text == bs.cancel:
            ...
            user.delete_instance()
            return False
        if message.text == bs.i_designer:
            bot.send_message(sender_id, 'Ваша заявка принята.')
            keyboard = types.InlineKeyboardMarkup()
            user = Admin.select().where(Admin.superuser == 1).get()
            if user:
                buttons = types.InlineKeyboardButton(text="Добавить " + str(username) + " ( " + str(first_name) + " "+ str(last_name) +") ",
                                                     callback_data="add_designer " + str(sender_id))
                keyboard.add(buttons)
                bot.send_message(user.user_id, "Заявка на дизайнера", reply_markup=keyboard)
        try:
            user = User.select().where(User.user_id == sender_id).get()
            step = user.step
        except:
